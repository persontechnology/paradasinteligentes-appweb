						...
						if (response.type === 'success') {
							...
							if (typeof options.success === 'function') {
								...
								var success = function () {
									...
									var $successMessage = $('<div class="quform-success-message d-block text-center text-success p-2"/>').html(response.message).hide().insertBefore(self.$container).fadeIn(options.successFadeInSpeed).show(0, function() {
										// successEnd function
								...
								$.each(response.elementErrors, function(index, info) {
									...
									if (typeof info.errors === 'object' && info.errors.length > 0) {
										...
										if ($element.length) {
											...
											// Append the error
											$errorList.append('<div class="quform-error form-text text-danger">' + info.errors[0] + '</div>');
